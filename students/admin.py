...
class GroupListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        groups = Groups.objects.all().order_by('name_group')
        lst = [(group.pk, group.name_group) for group in groups]
    ...
    def queryset(self, request, queryset):
        match self.value():
            ...
            case _:
                return Student.objects.filter(group=Groups.objects.get(pk=int(self.value())))
...
class StudentAdmin(admin.ModelAdmin):
    ...
    def group_name(self, instance):
        if instance.group:
            return instance.group.name_group
